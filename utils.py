...
def feature_tokenize(text, f_sep=u"￨"):
    # returns words and features (0 or 1) seperated 
    # example => I |0 went |0 to |0 the |0 USA |1 => [I, went, to, the, USA] [0, 0, 0, 0, 1] 
...
def correct_tokens(pred, true_tokens, padding_idx):
    ...
    # torch.ne() returns a boolean vector where element is True if that index's elements in both tensors aren't equal
    non_padding = true_tokens.view(-1).ne(padding_idx)
    # masked_select selects only those elements where the element in non_padding is True
